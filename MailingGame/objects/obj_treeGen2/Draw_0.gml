...
for (var i = 1; i < segments; i += 1) {
	if segment_array[i][3] != 0 {
		...
		if segment_array[i][3].place = 0 {
			draw_sprite_ext(spr_tree_leaves, segment_array[i][3].leaf, x + (lerp(-0.3, 0.3, i / segments) * segment_array[i][3].dir * segment_array[i][0]) + segment_array[i][1] + (segment_array[i][3].dir * segment_array[i][3].leaf_x), y - (20 * i) - segment_array[i][3].leaf_y, segment_array[i][3].dir, 1, 0, c_grey, 1);
		}
...
draw_sprite_ext(spr_tree_leaves, crown_leaf, x + segment_array[segments - 1][1], y - 20 * (segments) - 32, branch, 1, 0, c_white, 1);
