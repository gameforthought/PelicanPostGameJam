...
draw_vertex_texture(x + (sign(branching_array[i + 1] + 0.1) * (initial_width / 3)) + B_join + ((branching_array[0] - 1) * lerp(-20, 20, i)), y - branchA_height - branchB_height, 1, 0);
draw_vertex_texture(x - (sign(branching_array[i + 1] + 0.1) * (initial_width / 3)) + B_join  + ((branching_array[0] - 1) * lerp(-20, 20, i)), y - branchA_height - branchB_height, 0, 0);
draw_primitive_end();
}
...
for (var i = 0; i < branching_array[1]; i += 1) {
	draw_primitive_begin_texture(pr_trianglestrip, tex);
	draw_vertex_texture(x + ((initial_width / 3)) + B_join + ((branching_array[0] - 1) * -20), y - branchA_height - branchB_height, 1, 1);
	draw_vertex_texture(x - ((initial_width / 3)) + B_join  + ((branching_array[0] - 1) * -20), y - branchA_height - branchB_height, 0, 1);
