/// @description Insert description here
// You can write your code in this editor

var _surface_distort = surface_create(room_width,room_height);


surface_set_target(_surface_distort);

    draw_clear_alpha(COLOUR_FOR_NO_MOVE,0);

    // Anything we draw here will distort the screen
    for (var i = 0; i < room_width; i += 1) {
		draw_set_color(make_color_rgb(127, 127 + (20 * sin((1 / room_width) * pi * i)), 255));
		draw_rectangle(i, 0, i + 1, room_height, false);
	}

var _surface_texture_page = surface_get_texture(_surface_distort);

surface_reset_target();




var _post_distort = surface_create(windowScale * width, windowScale * height);
surface_set_target(_post_distort);

shader_set(sh_roundDistort);

    texture_set_stage(distortion_stage, _surface_texture_page);

	
	draw_surface(obj_night.app_surf,0,0);
	//draw_surface(application_surface,0,0);
	
    
shader_reset();

surface_free(_surface_distort); // always remember to remove the surface from memory 

surface_reset_target();

if (!surface_exists(srf_ping)) {
	srf_ping = surface_create(windowScale * width, windowScale * height);
}
if (!surface_exists(srf_pong)) {
	srf_pong = surface_create(windowScale * width, windowScale * height);
}

shader_set(sh_bloom);
	shader_set_uniform_f(u_bloom_threshold, 0.1);
	shader_set_uniform_f(u_bloom_range, 1);
	
	surface_set_target(srf_ping);
		draw_surface(_post_distort, 0, 0);
	surface_reset_target();
	
	gpu_set_tex_filter(true);
	
	shader_set(shader_blur);
		shader_set_uniform_f(u_blur_steps, 100);
		shader_set_uniform_f(u_sigma, 2);
		shader_set_uniform_f(u_blur_vector, 1, 0);
		shader_set_uniform_f(u_texel_size, texel_w, texel_h);
		
		surface_set_target(srf_pong);
			draw_surface(srf_ping, 0, 0);
		surface_reset_target();
		
		shader_set_uniform_f(u_blur_vector, 0, 1);
		
		surface_set_target(srf_ping);
			draw_surface(srf_pong, 0, 0);
		surface_reset_target();
		
	gpu_set_tex_filter(false);
		
		
shader_reset();
gpu_set_tex_filter(false);

draw_surface(_post_distort, 0, 0);
gpu_set_blendmode(bm_add);
draw_surface(srf_ping, 0, 0);
gpu_set_blendmode(bm_normal);

surface_free(srf_ping);
surface_free(srf_pong);



